 ...
 */
class BreadcrumbsMicro extends Breadcrumbs
{
    ...
    public function run()
    {
        ...
        $links = [];
        $replacement = ['>{link}' => ' itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">{link}{position}'.'<span class="itemSeparator">/</span>'];
        //todo: , '<img ' => '<img itemprop="image" '
        $this->itemTemplate = strtr($this->itemTemplate, $replacement);
        $this->activeItemTemplate = strtr($this->activeItemTemplate, ['>{link}' => ' >{link}']);
