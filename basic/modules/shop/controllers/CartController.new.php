...
class CartController extends Controller
{
    ...
    //TO DO to look
    public function actionResult($id = null, $param = null, $ok = null){
        ...
        if ($order->load(Yii::$app->request->post()) && !empty($p)) {
            ...
            if($order->payment_choose == 2){
                ...
                $data = [
                    ...
                    'returnUrl' => 'https://apfy.ru/shop/cart/sbersucces',
                    'failUrl' => 'https://apfy.ru/shop/cart/sbersucces',
                    'sessionsTimeoutSecs' => 1200,
                    'description' => 'Заказ в магазине https://apfy.ru',

                ];

                $result = $this->sendApi('registerPreAuth.do', $data);
                //$result = $this->sendApi('register.do', $data);
                /*var_dump($result);
                exit;*/
            ...
            if($order->payment_choose == 3){
                ...
                if(array_key_exists('formUrl', $result)) {
                    ...
                    $invoice->save(false);
                    var_dump($result); var_dump($formUrl); exit;
                    return $this->redirect($formUrl);
                }
    ...
    private function sendApiAlfa($action, $data)
    {
        ...
        //$url = 'https://pay.alfabank.ru/payment' . $action;//боевой сервер
