...
<yml_catalog date="<?=date ("Y-m-d H:m")?>">
    <shop>
        ...
        <categories>
            <category id="86" >Каталог</category>
            <category id="1075" parentId="86">Товары для активного отдыха</category>
            <?php foreach ($treeCat as $k=>$v): ?>
                <category id="<?=$k?>" <?php if($v['parent_id'] !== null) echo 'parentId="'.$v['parent_id'].'"'; else  echo 'parentId="1075"';?>><?=$v['name']?></category>
            <?php endforeach; ?>
        </categories>
        <delivery-options>
