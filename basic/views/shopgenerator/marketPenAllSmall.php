    ...
    <shop>
        ...
        <offers>
            <?php foreach ($products_yml as $k => $v):?>
                <?php
                //var_dump($v); exit;
                //if(empty($v['category']))) Continue;
                if(empty($treeCat[$v['category']])) {
                    continue;
                    //var_dump($v); exit;
                    //var_dump($treeCat[$v['category']]['name']); exit;
                }
                $isAr = [
                    475,
                    476
                ];
                //if($treeCat[$v['category']]['name']) continue;
                if(in_array($v['id'], $isAr)) Continue;
                if($v['price'] > 300) Continue;
                ?>
                <offer id="<?=$v['id']?>" available="true" bid="1" fee="700">
