//шаблон товара в карзине
// var cartTmp = function(data, href){
//     return '<section class="item cart_element cart_item_id_'+data.id+'"><div class="container-fluid"><div class="row"><div class="col-sm-4"><img src="'+data.image+'" class="row"/></div><div class="col-sm-8"><a href="'+href+'" class="item_href transition_02" title="'+data.name+'">'+data.name+'</a><div class="cart_item_price"><span class="item_change_quant quant_add_to transition_02" data-action="add" data-id="'+data.id+'"><i class="fa fa-plus-circle" aria-hidden="true"></i></span><span class="cart_item_quant">'+data.quantity+'</span><span class="item_change_quant quant_remove_to transition_02" data-action="remove" data-id="'+data.id+'"><i class="fa fa-minus-circle" aria-hidden="true"></i></span> X <span class="item_price">'+data.price+'</span></div></div></div></div><span class="cart_item_delete fa fa-times-circle transition_02" aria-hidden="true" data-id="'+data.id+'"></span> </section>';
// }
var cartTmp = function(data, href){
...
//отображение пустой или полной карзины
$('#cart_items').toggleClass('visible_hidden', CartObj.arrayItems.length>0);

//добавление товара из категорий продуктов
...
//Удаление товара из карзины окончательно
...
//удаление товара через карзину, по одному
